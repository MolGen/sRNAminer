...
shinyServer(function(input, output) {
  ...
  table <- reactive({
    gr <- gr()
    table <- data.frame(chr = input$chr, start = start(gr) -1 , end = end(gr), name = names(gr), 
                        score = "0", strand = strand(gr))
    table
  })
...
#    writeLines(paste("# Generated by sRNA miner, ", format(Sys.Date(), "%c"), sep = " "), file)
    write.table(table(), file, sep = "\t", quote = F, row.names = F, col.names = F, append = TRUE)
  })
})
